...
# Turbeaux Sounds KEYQ Installer Script

echo "Installing Turbeaux Sounds KEYQ..."
...
# Install Audio Unit
if [ -d "$SCRIPT_DIR/KEYQAudioUnit.component" ]; then
    echo "Installing Audio Unit plugin to $COMPONENTS_DIR..."
    cp -R "$SCRIPT_DIR/KEYQAudioUnit.component" "$COMPONENTS_DIR/"
...
if [ -d "$SCRIPT_DIR/KEYQStandalone.app" ]; then
    echo "Installing Standalone app to /Applications..."
    cp -R "$SCRIPT_DIR/KEYQStandalone.app" /Applications/
...
echo "Please restart Logic Pro to use the Turbeaux Sounds KEYQ plugin."
echo ""
